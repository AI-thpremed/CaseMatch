    ...
    def log(msg, level="INFO"):
        if msg is None:
            ...
            if log_file:
                log_file.close()
            return
        formatted = msg if level == "INFO" else f"[{level}] {msg}"
        if log_file:
...
#                 feats = existing_feats
#             else:
#                 # --- Extract features for added images ---
#                 added_feats = []
#
#                 if added_paths:
#                     handle = None
...
#
#                     with torch.no_grad():
#                         for i, img_path in enumerate(added_paths):
#                             try:
#                                 img = Image.open(img_path).convert('RGB')
...
#
#                                 if (i + 1) % 50 == 0 or (i + 1) == len(added_paths):
#                                     log(f"[{s_name}] Processed {i + 1}/{len(added_paths)} added images")
#
#
#                             except Exception as e:
#                                 log(f"[{s_name}] Skipped {img_path}: {str(e)}", "WARNING")
...
#
#                     if added_feats:
#                         feats = np.vstack([kept_feats, np.array(added_feats, dtype=np.float32)])
#                         all_paths = kept_paths + added_paths
#                     else:
#                         feats = kept_feats
#                         all_paths = kept_paths
#                 else:
#                     if added_feats:
#                         feats = np.array(added_feats, dtype=np.float32)
